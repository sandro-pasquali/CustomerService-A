...
$xml->element('div', $rootLabel, array('class' => 'subElementLabel', 'state' => 'closed', 'icon' => 'folder', 'id' => ''));
...
foreach($parents as $pID => $data)
  {
    ...
    else
      {
        ...
        $xml->element('div', $data['info']['label'], array('class' => 'subElementLabel', 'state' => 'empty', 'id' => $itemID, 'icon' => $icon));
        $xml->pop();
      }
  }
